...
#Arbir launcher
subprocess.Popen(["adb shell input swipe 400 880 400 50 700"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
time.sleep(2)
...
#estado bateria
script = subprocess.Popen(["adb shell dumpsys battery | grep level"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
(out, err) = script.communicate()
with open("index.html", 'a') as archivo:
    archivo.write("<p>Nivel de bateria = " + str(out.decode('utf-8')) +  "</p>")
time.sleep(2)


#activar bluetooth
script = subprocess.Popen(["adb shell settings get global bluetooth_on "], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
(out, err) = script.communicate()
with open("index.html", 'a') as archivo:
    archivo.write("<p>Bluetooth = " + str(out.decode('utf-8')) +  "</p>")
time.sleep(2)
